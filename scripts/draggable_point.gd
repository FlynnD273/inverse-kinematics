...
func _on_mouse_entered() -> void:
  ...
  tween.tween_property(self, "scale", Vector2.ONE * 1.2, 0.5).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)


func _on_mouse_exited() -> void:
  var tween := create_tween()
  tween.tween_property(self, "scale", Vector2.ONE, 0.5).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
