...
func _process(_delta: float) -> void:
  if ($End.position - $Start.position).length() > (points.size() - 1) * joint_length:
    if $End.is_dragging:
      $Start.position = $End.position + ($Start.position - $End.position).normalized() * joint_length * (points.size() - 1)
    else:
      $End.position = $Start.position + ($End.position - $Start.position).normalized() * joint_length * (points.size() - 1)
  points[0].hide()
  points[-1].hide()
  ...
  for _iter in range(iteration_count):
    ...
    for i in range(points.size() - 2, -1, -1):
      points[i].position = points[i+1].position + (points[i].position - points[i + 1].position).normalized() * joint_length
